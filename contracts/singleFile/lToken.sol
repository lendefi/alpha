...
interface ComptrollerInterface {
    ...
    function buyAllowed(address borrowAsset, address buyAsset, uint borrowAmount, uint partialAmount) external returns (bool);
    function proxyMint(address cToken, address payable beneficiary, uint borrowAmount) external returns (uint);
    function enterMarketsForUser(address[] calldata cTokens, address user) external returns (uint[] memory);
}
...
contract CToken is EIP20Interface, Exponential, TokenErrorReporter, ReentrancyGuard {
     ...
     */
    function mintInternal(uint mintAmount, address minter, address beneficiary) internal nonReentrant returns (uint) {
        ...
        return mintFresh(minter, beneficiary, mintAmount);
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function mintFresh(address minter, address beneficiary, uint mintAmount) internal returns (uint) {
        ...
        vars.err = checkTransferIn(beneficiary, mintAmount);
         ...
         */
        vars.err = doTransferIn(beneficiary, mintAmount);
        if (vars.err != Error.NO_ERROR) {
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function borrowInternal(uint borrowAmount, address payable beneficiary, bool credited) internal nonReentrant returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted borrow failed
        ...
        return borrowFresh(beneficiary, borrowAmount, credited);
    }
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function borrowFresh(address payable borrower, uint borrowAmount, bool credited) internal returns (uint) {
